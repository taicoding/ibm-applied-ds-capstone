...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider',
                                        ...
                                        marks={x:x for x in range(0,10000,1000)},
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # Contar éxitos por sitio
        filtered_df = pd.DataFrame(spacex_df.groupby('Launch Site')['class'].sum().reset_index())
        fig = px.pie(filtered_df, values='class', names='Launch Site', title='Total Successful Launches by Site')
        return fig
    else:
        # Contar éxitos y fracasos para el sitio seleccionado
...
def get_scatter(site, payload):
    if site == 'ALL':
        df_filtered = spacex_df[(spacex_df['Payload Mass (kg)'] <= payload[1]) & (spacex_df['Payload Mass (kg)'] >= payload[0])]
        ...
        df_filtered = spacex_df[(spacex_df['Launch Site'] == site) & ((spacex_df['Payload Mass (kg)'] <= payload[1]) & (spacex_df['Payload Mass (kg)'] >= payload[0]))]
        fig = px.scatter(df_filtered, 
        x = 'Payload Mass (kg)', 
        y = 'class', 
        color = "Booster Version Category",
